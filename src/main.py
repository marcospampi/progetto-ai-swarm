...
mymap = Map(25,25)
bot = Agente(Position(10, 10), Visibility(3, 0, False), 100, Map(25, 25, -1), Strategy(1,1, 10, 0.8))
mymap.grid[12][12] = 3
t = 100
...
def loop():
    ...
    bot.local_map.set_cell((bot.position.x, bot.position.y), 0)
    bot.action(mymap)
    bot.local_map.set_cell((bot.position.x, bot.position.y), 6)
    mymap.set_cell((bot.position.x, bot.position.y), 6)
    bot.local_map.print_map()
